...
Use the existing `IPortableGeometryPathSource`, `IPortableGuidelineSetSource`, `IPortableTransformMatrixSource`, `IPortableBrushSource`, and `IPortablePenSource` seams as the pattern for future cleanup: expose narrow typed contracts from source-built WPF internals, keep DTOs package-neutral, and update tests to assert that hot-path readers do not use `System.Reflection`, `BindingFlags`, property probing, or duck-typed fake shapes.
