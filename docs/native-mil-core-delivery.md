...
preserving owner precedence and primary fallback. Backend and managed consumer
...
[ProGPU #139](https://github.com/wieslawsoltes/ProGPU/pull/139) is merged to
`main` at `86f2f766d1f8e6b4041fa184de0fe9d03ae2840f`. That merge tree equals
...
D3D12, Metal and Vulkan. [LibreWinForms #29](https://github.com/wieslawsoltes/LibreWinForms/pull/29)
passed 7/7 checks and merged at `625befd5f7140343f80dbcf3185c23f15ca21318`;
...
immediately after the receiving host's GPU input query. Native cache diagnostics
...
| 2 — macOS source diagnostics complete | Application closure: native point/region owner queries use the presented scene and actual popup/floating host. Current-source Showcase and Toolkit complete their full macOS live actions; Showcase's 120-frame native performance/memory gate and seven native WPF hit fixtures pass. These mixed-source runs do not qualify packaged startup, Windows SDK, Linux/X11/Wayland, license-controlled Xceed/SciChart or default admission. Treat any failure in those required actions as a concrete reopened implementation blocker. | Finish the remaining application actions on exact packaged builds, preserving separate-host device input, popup ownership, resize/DPI, content/effect/cache and close/reopen/device-loss contracts. |
| 3 — current | Final qualification and delivery: build complete platform artifacts and packages, execute cross-platform/Windows comparisons and application gates, fix failures, and bring all three PRs' required CI to green at the delivery commits. | Record exact-head package consumption and required gate results, with explicit failures or environment/license limitations. Only then report the core release delivered. |
...
The [native host query connection](../reports/native-mil-host-query-routing-2026-09-09.md)
routes point/region owners and candidates through presented native snapshots,
inherits native input admission for popups, and obtains actual index counts/GPU
