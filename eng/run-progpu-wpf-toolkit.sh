...
if [[ "${PROGPU_WPF_TOOLKIT_LIVE_VALIDATE:-0}" == "1" ]]; then
  ...
  swapchain_line=""
  for _ in {1..600}; do
    live_validation_line="$(grep -E "ProGPU WPF Toolkit live input validation succeeded:" "${live_log}" | tail -n 1 || true)"
  ...
  if [[ -z "${swapchain_line}" ]]; then
    echo "Expected Toolkit apphost to configure a ProGPU swapchain." >&2
    cat "${live_log}" >&2
    exit 1
  fi
  ...
  if [[ "${live_validation_line}" =~ logical[[:space:]]([0-9]+)x([0-9]+),[[:space:]]pixels[[:space:]]([0-9]+)x([0-9]+),[[:space:]]viewport[[:space:]]([0-9]+)x([0-9]+)@([0-9]+),([0-9]+),[[:space:]]dpi[[:space:]]([0-9]+(\.[0-9]+)?) ]]; then
    ...
    echo "Could not parse Toolkit apphost geometry from live validation or swapchain line." >&2
    cat "${live_log}" >&2
    exit 1
  fi
