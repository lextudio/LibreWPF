...
For mutable development package versions such as `0.1.0-preview.20`, the SDK clears known WPF and ProGPU runtime assemblies from the app output before recopying package assets. This prevents an incremental app rebuild from launching stale bridge/compositor DLLs after a local package refresh while preserving normal incremental copy behavior for stable package versions. Set `ProGpuWpfClearMutablePackageOutputs=false` to disable this development safeguard.
...
```xml
<Project Sdk="LibreWPF.Sdk/0.1.0-preview.20">
  <PropertyGroup>
...
The quickcheck expects the local `0.1.0-preview.20` package feed to be current. Use the full SDK CI gate when package contents need to be rebuilt from source:
