...
- The real XAML runtime harness can now focus the shown portable root through WPF's own `Keyboard.Focus(...)` over `PortablePresentationSource`; child `TextBox` keyboard focus still waits on the remaining non-HWND window template/content-hosting visibility work, not a managed-code rewrite.
