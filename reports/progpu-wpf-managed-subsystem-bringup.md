...
- Built `ProGPU.Wpf.Tests` and ran `WpfManagedProjectGraphTests`; 28 focused graph tests cover the real `PresentationFramework` harness, the real XAML compiler/resource-dictionary/event-hookup harness, the real XAML runtime merged-dictionary/generated-event/dynamic-binding/direct-command/routed-command/control-template/dynamic-resource/style-trigger/item-template/selection/TextBox-focus harness, the real `Application.Run` merged-dictionary/generated-event/dynamic-binding/direct-command/routed-command/control-template/dynamic-resource/style-trigger/item-template/selection harness, the real Fluent theme runtime harness, the retained ProGPU theme replay/layout check, the `FlowDocumentView` native PTS guard, managed font/classification/LineServices/typeface bootstrap guards, and native-entrypoint guard coverage.
