...
`WpfManagedProjectGraphTests.RealThemeRuntimeHarnessLoadsFluentThemeBamlThroughRealPresentationFramework` now locks down the first real theme runtime smoke. `src/ProGPU.Wpf.RealThemeRuntimeHarness` builds `PresentationFramework.Fluent`, loads `/PresentationFramework.Fluent;component/Themes/Fluent.xaml` through real `ResourceDictionary.Source`, merges it into the compiled app resources, applies representative `Window`, `Button`, and `RichTextBox` Fluent styles/templates, verifies the themed `Window` and `Button` templates can apply without Windows native DLLs, and attaches the themed XAML-created `Window` to portable ProGPU activation. The harness deliberately validates the `RichTextBox` Fluent template as style data without calling `RichTextBox.ApplyTemplate()` yet: that call enters WPF's PTS/LineServices formatter and still requires a cross-platform document layout replacement for `PresentationNative_cor3.dll`.
...
- Built and ran `ProGPU.Wpf.RealThemeRuntimeHarness`; the harness loaded real Fluent theme BAML through `ResourceDictionary.Source`, applied representative `Window`, `Button`, and `RichTextBox` theme resources, and attached the themed XAML `Window` to portable ProGPU activation.
- Built `ProGPU.Wpf.Tests` and ran `WpfManagedProjectGraphTests`; 26 focused graph tests passed, covering the real `PresentationFramework` harness, the real XAML compiler harness, the real XAML runtime harness, the real Fluent theme runtime harness, and native-entrypoint guard coverage.
