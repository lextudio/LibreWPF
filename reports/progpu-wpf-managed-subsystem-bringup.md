...
- UI Automation identifier initialization now has non-Windows guards for the shared OS-version helper and reserved-value probes. Basic selector/list controls can initialize automation metadata without loading `PresentationNative` or `UIAutomationCore`, while full provider/client integration remains a later platform-services item.
