...
ProGPU now has a native WPF-style shader-effect execution target in `CompositorBuiltInExtensions.WpfShaderEffect`. The extension compiles WGSL into cached WebGPU render pipelines with explicit compositor-compatible bind group layouts, samples a source texture, exposes WPF-style constants as 32 `vec4<f32>` registers, and supplies bounds/texture-size uniforms. The MIL decoder and reflection bridge still count WPF effect scopes as unsupported until WPF effect resources can be extracted and mapped to this descriptor, and until legacy Direct3D pixel-shader bytecode has a translation or replacement registry.
