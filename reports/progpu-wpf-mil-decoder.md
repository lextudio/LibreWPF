...
ProGPU now has a native WPF-style shader-effect execution target in `CompositorBuiltInExtensions.WpfShaderEffect`. The extension compiles WGSL into cached WebGPU render pipelines with explicit compositor-compatible bind group layouts, samples a source texture, exposes WPF-style constants as 32 `vec4<f32>` registers, and supplies bounds/texture-size uniforms. The visual reflection bridge can also convert WPF-shaped `BlurEffect` and `DropShadowEffect` state into ProGPU-native effect descriptors when a sink implements `IWpfVisualEffectCommandSink`. The default flat ProGPU command sink still counts visual effects as unsupported because applying those descriptors correctly requires a retained visual/effect scope rather than inline draw-command mutation. The MIL decoder and source drawing-context effect scopes still count WPF effect scopes as unsupported until WPF effect resources can be extracted and mapped to native descriptors, and until legacy Direct3D pixel-shader bytecode has a translation or replacement registry.
