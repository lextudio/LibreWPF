...
ProGPU now has a native WPF-style shader-effect execution target in `CompositorBuiltInExtensions.WpfShaderEffect`. The extension compiles WGSL into cached WebGPU render pipelines with explicit compositor-compatible bind group layouts, supports WPF-style constants as 32 `vec4<f32>` registers, exposes a 16-slot sampler-register bank, and supplies bounds/texture-size uniforms. The visual reflection bridge converts WPF-shaped `BlurEffect` and `DropShadowEffect` state into ProGPU-native effect descriptors, and the Silk.NET host uses `ProGpuRetainedCompositionCommandSink` so `WpfRootVisual` replay can allocate bounded retained ProGPU effect visuals for those descriptors. The direct flat command sink still counts visual effects as unsupported because it has no retained visual/effect scope. The MIL decoder and source drawing-context effect scopes still count WPF effect scopes as unsupported until WPF effect resources can be extracted and mapped to native descriptors, and until legacy Direct3D pixel-shader bytecode has a translation or replacement registry.
