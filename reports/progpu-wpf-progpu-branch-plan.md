...
- shader-native ProGPU arc stroke strips in `ProGPU.Scene.Compositor`/`ProGPU.Backend.Shaders`, using reusable `ProGPU.Vector.ArcShaderParameters` created by `ArcSegmentGeometry.TryCreateShaderParameters(...)` to evaluate stroke vertices from transformed ellipse axes in WGSL instead of flattening arcs to line segments.
...
- Composition: build on retained `ChangeVersion` counters with resource-version snapshots and frame coalescing policy so WPF `Freezable`/visual invalidation can skip unchanged retained branches without reflection polling. Keep native offscreen effect and layer rendering version-stable by using render-only transform overrides rather than mutating retained visual properties during composition.
