...
- native `ProGPU.Scene` WPF shader-effect execution primitives: `WpfShaderEffectParams`, `DrawWpfShaderEffect(...)`, `WpfShaderEffect`, and `WpfShaderEffectExtensionPipeline` for WGSL/WebGPU effects with WPF-style constant registers, a fixed 16-slot sampler-register bank, configurable visual-source sampler register mapping for retained `Visual.Effect` usage, explicit compositor-compatible pipeline layouts, cached native bind groups/render pipelines, transparent fallback bindings for unbound samplers, and headless render-path coverage.
...
- Effects: continue mapping WPF effect resources onto ProGPU-native shader/compute/render pipelines. Visual `BlurEffect` and `DropShadowEffect` descriptors can now flow through retained ProGPU host replay, and registered WPF `ShaderEffect` replacements can now lower to visual-level `ProGPU.Scene.WpfShaderEffect` descriptors that run WGSL over the retained visual's offscreen source through native WebGPU with WPF sampler-register mapping. Remaining WPF-facing work is generated `PushEffect` resource mapping, non-implicit sampler brush/texture inputs, and legacy Direct3D pixel-shader bytecode import/translation so existing WPF HLSL effects can populate the native ProGPU shader descriptor automatically. Avoid managed CPU shader emulation.
