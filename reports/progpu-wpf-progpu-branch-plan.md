...
- Text: build on the reusable SFNT and simple shaper helpers by adding richer OpenType layout table APIs, full glyph shaping hooks, fallback chains, and exact subset writing so WPF `PortableTextInterface` can delegate more of the remaining text stack instead of carrying compatibility code.
...
- Replace the local `PortableTextInterface` text stack with signed `ProGPU.Text` services incrementally: face-offset/table/cmap/glyph-metric discovery and the simple one-scalar-to-one-glyph fallback now use ProGPU-owned source-shared helpers, while full shaping, fallback-chain policy, and subsetting should move as the ProGPU text APIs cover those surfaces.
