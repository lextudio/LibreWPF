...
- Effects: map WPF `BlurEffect`, `DropShadowEffect`, and `ShaderEffect` resources onto ProGPU-native shader/compute/render pipelines. The native shader-effect sampler bank now supports WPF sampler-register descriptors directly; remaining WPF bridge work should only extract descriptors/resources and schedule the effect. Legacy Direct3D pixel-shader bytecode must use a translation pipeline or explicit replacement registry because WebGPU cannot execute it directly. Avoid managed CPU shader emulation.
