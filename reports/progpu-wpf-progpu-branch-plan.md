...
- native `ProGPU.Scene` WPF shader-effect execution primitives: `WpfShaderEffectParams`, `DrawWpfShaderEffect(...)`, `WpfShaderEffect`, and `WpfShaderEffectExtensionPipeline` for WGSL/WebGPU effects with WPF-style constant registers, a fixed 16-slot sampler-register bank, visual-source sampler register 0 for retained `Visual.Effect` usage, explicit compositor-compatible pipeline layouts, cached native bind groups/render pipelines, transparent fallback bindings for unbound samplers, and headless render-path coverage.
