...
- Use signed ProGPU assemblies for direct references from strong-named WPF assemblies. Do not reintroduce unsigned helper assemblies into the root WPF build graph.

## ProGPU Features to Implement Next

- Text: expose a reusable ProGPU text service that covers SFNT font loading, cmap/metrics, OpenType layout table access, glyph shaping hooks, glyph placement, fallback chains, and exact subset writing so WPF `PortableTextInterface` can delegate instead of carrying a duplicate parser.
...
- Replace the local `PortableTextInterface` SFNT reader with signed `ProGPU.Text` services once the text service has WPF-required shaping/subsetting coverage.
