...
- native `ProGPU.Scene` WPF shader-effect execution primitives: `WpfShaderEffectParams`, `DrawWpfShaderEffect(...)`, and `WpfShaderEffectExtensionPipeline` for WGSL/WebGPU source-texture effects with WPF-style constant registers, explicit compositor-compatible pipeline layouts, and headless render-path coverage.
...
- Effects: map WPF `BlurEffect`, `DropShadowEffect`, and `ShaderEffect` resources onto ProGPU-native shader/compute/render pipelines. Managed WPF code should only extract descriptors/resources and schedule the effect; legacy Direct3D pixel-shader bytecode must use a translation pipeline or explicit replacement registry because WebGPU cannot execute it directly. Avoid managed CPU shader emulation.
