...
The remaining generator tooling limitation is schema regeneration for `ResourceModel/Generated/Elements.cs`: Windows still uses `xsd.exe` through `GenerateElements.cmd`; the non-Windows path currently relies on the checked-in generated schema model. Add a managed XSD-codegen replacement before changing `xml/Elements.xsd` on a non-Windows lane.

## Generator Migration

The long-term change should complete the ProGPU generated path:

1. Keep `System/Windows/Media/Generated/RenderDataDrawingContext.cs` in sync with `src/WpfGfx/codegen/mcg/generators/renderdata.cs` by running `dotnet build src/Microsoft.DotNet.Wpf/src/WpfGfx/codegen/mcg/mcg.proj -p:McgOutputDirectory=<output>` on non-Windows before copying generated changes into source. Use a Windows lane or a future managed XSD-codegen replacement when `xml/Elements.xsd` changes.
