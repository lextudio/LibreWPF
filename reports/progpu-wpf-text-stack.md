...
The real Fluent theme/runtime smoke now exercises more of the managed text bootstrap while applying real control templates. On non-Windows, `FontCache.FileMapping` can open local font files through a managed pinned-buffer stream, `MS.Internal.Classification` has a conservative managed Unicode classification fallback for basic scripts/categories, `LoGetEscString` supplies managed pointers for the special LineServices characters, and `Typeface.CheckFastPathNominalGlyphs` keeps WPF's existing `SimpleTextLine` path available for basic nominal-glyph layout after complex characters are excluded. This lets the themed XAML smoke measure and arrange basic `TextBlock`/`TextBox` content without native LineServices. It deliberately stops short of claiming formatted-text parity: complex/full formatting still enters native `LoCreateContext`, so full paragraph layout, bidi line services, caret stops, and rich document formatting remain open.
