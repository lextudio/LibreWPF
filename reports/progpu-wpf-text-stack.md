...
The first runtime text slice is now implemented for file-backed SFNT fonts. `PortableTextInterface` can enumerate WPF `IFontSourceCollection` entries and platform font folders, load `.ttf`, `.otf`, and `.ttc` files, parse TTC face offsets, read SFNT tables, expose localized `name` strings, read `head`/`hhea`/`maxp`/`hmtx`/`OS/2`/`post` metrics, map Unicode code points through cmap format 4 and 12, return glyph advances and basic bounds from `glyf`/`loca` when present, expose embedding rights, and hand GSUB/GPOS/GDEF table bytes to WPF's existing OpenType layout cache. Direct `ProGPU.Text` project references were deferred because root WPF assemblies remain on the Arcade strong-name path while ProGPU assemblies were unsigned. The ProGPU submodule branch now strong-name signs its assemblies with public key token `c29c9752855ee183`, so a direct `ProGPU.Text` replacement path is now unblocked as a next implementation step; the current portable parser remains self-contained until the text code is deliberately moved behind the ProGPU text API.
...
The current implementation keeps a self-contained managed SFNT reader inside `PresentationCore` for type-identity and incremental-port reasons. Future implementations can replace or share this with signed ProGPU `TtfFont`/text services now that the strong-name boundary is removed. More complete shaping should be backed by a portable shaping engine before formatted text fidelity is claimed.
...
- Replace the remaining portable `MS.Internal.Text.TextInterface` simple glyph fallback with real signed ProGPU-backed or portable shaping logic. File-backed font discovery, glyph mapping, basic metrics, embedding rights, OpenType layout table access, and one-scalar-to-one-glyph advances now have managed implementations.
