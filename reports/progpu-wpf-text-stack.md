...
The current implementation keeps WPF-specific font wrapper types and subsetting code inside `PresentationCore` for type-identity and incremental-port reasons. SFNT metadata, table, cmap, glyph metric, glyph bounds, and embedding-right discovery now come from the ProGPU-owned source-shared `SfntFontFace` helper. The simple one-scalar-to-one-glyph analyzer fallback now comes from the ProGPU-owned source-shared `SfntSimpleGlyphShaper` helper. Future implementations should replace that simple fallback with full shaping, fallback-chain policy, and exact subsetting as ProGPU exposes those APIs. More complete shaping should be backed by a portable shaping engine before formatted text fidelity is claimed.
...
- `PresentationCore` source-includes `external/ProGPU/src/ProGPU.Text/SfntFontFace.cs` and `external/ProGPU/src/ProGPU.Text/SfntSimpleGlyphShaper.cs` on non-Windows; the helpers are public when built by ProGPU.Text and internal when linked into WPF so ApiCompat does not expose ProGPU types as WPF API.
...
- Replace the source-shared simple one-scalar-to-one-glyph fallback with real signed ProGPU-backed or portable shaping logic. File-backed font discovery, glyph mapping, basic metrics, embedding rights, OpenType layout table access, cluster maps, and one-scalar-to-one-glyph advances now have managed implementations through ProGPU-owned helpers where the APIs are backend reusable.
