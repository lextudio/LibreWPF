...
The real Fluent theme/runtime smoke now exercises more of the managed text bootstrap while applying real control templates. On non-Windows, `FontCache.FileMapping` can open local font files through a managed pinned-buffer stream, `MS.Internal.Classification` has a conservative managed Unicode classification fallback for basic scripts/categories, and `LoGetEscString` supplies managed pointers for the special LineServices characters. This deliberately stops short of claiming formatted-text parity: `TextFormatterContext` still enters native `LoCreateContext`, so full paragraph layout, bidi line services, caret stops, and rich document formatting remain open.
...
- Decide whether LineServices is replaced directly, wrapped behind a compatibility layer, or bypassed for the first ProGPU formatted-text path. The current managed non-Windows bootstraps cover classification and escape-string tables only; `LoCreateContext` and paragraph line formatting are still native boundaries.
