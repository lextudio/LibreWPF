...
The current implementation keeps metrics, cmap, glyph bounds, and subsetting code inside `PresentationCore` for type-identity and incremental-port reasons. Metadata/table discovery now comes from the ProGPU-owned source-shared `SfntFontFace` helper. Future implementations should move glyph metrics, shaping, fallback, and exact subsetting as ProGPU exposes those APIs. More complete shaping should be backed by a portable shaping engine before formatted text fidelity is claimed.
...
- Replace the remaining portable `MS.Internal.Text.TextInterface` simple glyph fallback with real signed ProGPU-backed or portable shaping logic. File-backed font discovery, glyph mapping, basic metrics, embedding rights, OpenType layout table access, and one-scalar-to-one-glyph advances now have managed implementations, and ProGPU now has the first reusable SFNT metadata/table API needed to start removing duplicate WPF-side parser code.
