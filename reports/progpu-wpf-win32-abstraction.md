...
- Real WPF `PortableWindowActivationService`: `PresentationFramework` registration and lifetime boundary for non-Windows `Window.Show` and `Application.Run`; registered delegates create the portable host/source, handle show/hide/run, and close/dispose the activation without a WPF-to-ProGPU compile-time dependency.
- `WpfPortableWindowActivation`: transition bootstrap that derives `ProGpuWpfWindowOptions` from WPF-shaped window `Title`/size properties, attaches the host-owned portable source, uses the top-level window as the root visual, registers the real `PortableWindowActivationService` factory by reflection, and maps native host close callbacks back to WPF-shaped `Close()`.
...
2. Complete portable window lifecycle integration beyond the first `Window.Show` and `Application.Run` bridge: hide/minimize behavior, close-cancellation semantics, app shutdown edge cases, and host disposal ordering.
