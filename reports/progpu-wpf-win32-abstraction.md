...
- Real WPF `PortableWindowActivationService`: `PresentationFramework` registration and lifetime boundary for non-Windows `Window.Show`; registered delegates create the portable host/source, handle show/hide, and close/dispose the activation without a WPF-to-ProGPU compile-time dependency.
- `WpfPortableWindowActivation`: transition bootstrap that derives `ProGpuWpfWindowOptions` from WPF-shaped window `Title`/size properties, attaches the host-owned portable source, uses the top-level window as the root visual, and can register the real `PortableWindowActivationService` factory by reflection.
...
2. Complete portable window event-loop and native lifecycle integration after the first `Window.Show` activation boundary: `Application.Run` ownership, native close callbacks, hide/minimize behavior, app shutdown, and host disposal ordering.
