...
2. Add WPF input manager routing, IME/stylus handling, full routed drag/drop over `IWpfDragDropService`, map real WPF `Dispatcher`/`DispatcherTimer`/`MediaContext` onto the queued dispatcher and timer boundaries, add native event-loop wakeups for scheduled renders, and deepen activation integration behind local interfaces.
