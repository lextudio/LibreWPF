...
2. Promote the host-owned `PortablePresentationSource` attachment APIs into the real non-Windows `Window` and `Application` startup path so Silk.NET size, DPI, input, and ProGPU presentation are created by WPF lifetime rather than test or host bootstrap code.
