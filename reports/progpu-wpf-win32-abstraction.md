...
- Input: `SilkNetWpfInputService` for low-level keyboard, text, mouse, and wheel events, `ProGpuWpfWindowHost.InputReceived` for host delivery, and `WpfPortableWindowActivation` for explicit transition-handler forwarding; WPF input-manager routing, keyboard focus scopes, IME, and stylus remain higher-level porting steps.
...
2. Complete portable window lifecycle integration beyond the first `Window.Show`, `Hide`, `WindowState`, title/client-size updates, focus-derived `Activated`/`Deactivated`, normalized input payload forwarding, `Closing` cancellation, and `Application.Run` bridge: keyboard focus behavior, app shutdown edge cases, and host disposal ordering.
