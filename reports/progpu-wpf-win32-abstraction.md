...
2. Complete portable window lifecycle integration beyond the first `Window.Show`, `Hide`, `WindowState`, title/client-size updates, focus-derived `Activated`/`Deactivated`, `Closing` cancellation, and `Application.Run` bridge: keyboard focus behavior, app shutdown edge cases, and host disposal ordering.
