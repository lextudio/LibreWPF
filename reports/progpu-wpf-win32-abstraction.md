...
- Real WPF `PortableWindowActivationService`: `PresentationFramework` registration and lifetime boundary for non-Windows `Window.Show`, `Hide`, `WindowState`, focus-derived `Activated`/`Deactivated`, `Closing`, and `Application.Run`; registered delegates create the portable host/source, handle show/hide/window-state/close/run, and close/dispose the activation without a WPF-to-ProGPU compile-time dependency.
- `WpfPortableWindowActivation`: transition bootstrap that derives `ProGpuWpfWindowOptions` from WPF-shaped window `Title`/size/window-state properties, attaches the host-owned portable source, uses the top-level window as the root visual, registers the real `PortableWindowActivationService` factory by reflection, maps native hide/minimize/maximize requests onto Silk.NET host state, maps focus-derived host activation/deactivation events back through WPF `Window.HandleActivate(bool)`, and maps native host close callbacks back to WPF-shaped `Close()` with cancel propagation through `IWindow.IsClosing`.
...
2. Complete portable window lifecycle integration beyond the first `Window.Show`, `Hide`, `WindowState`, focus-derived `Activated`/`Deactivated`, `Closing` cancellation, and `Application.Run` bridge: keyboard focus behavior, app shutdown edge cases, and host disposal ordering.
