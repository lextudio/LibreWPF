...
- Dispatcher posting and render wakeups: `QueuedWpfDispatcherService` queues callbacks by WPF-shaped priority and processes them on the owning thread from `ProGpuWpfWindowHost.ProcessDispatcherQueue`, `DoEvents`, and render callbacks; `IWpfRenderScheduler` preserves WPF-style invalidation requests until the host renders or its unchanged-frame gate can safely skip work.
- Timers: `ThreadPoolWpfTimerService` for portable one-shot and repeating timers used by future MediaContext, animation, hover, and dispatcher scheduling bridges.
...
2. Add WPF input manager routing, IME/stylus handling, full routed drag/drop over `IWpfDragDropService`, map real WPF `Dispatcher`/`DispatcherTimer`/`MediaContext` onto the queued dispatcher and timer boundaries, and deepen activation integration behind local interfaces.
