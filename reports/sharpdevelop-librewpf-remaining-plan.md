...
5. Broaden workbench service/runtime coverage.
   - Covered now: source-file save flows through the normal workbench save command, dirty-state transitions, temporary-file safe saving, and restore validation on the LineCounter sample.
   - Remaining: exercise debug commands, templates, add-in loading, command routing, broader workbench pads, toolbar updates, editor navigation, additional project loading cases, save-all/reload/external-change flows, and shutdown persistence.
...
- The save smoke remains green for a source file with temporary-file safe saving enabled and leaves the sample tree byte-for-byte clean after restore.
