...
- normal `App.xaml` / `MainWindow.xaml` with compiled startup/exit handlers and compiled merged resource dictionaries
...
Run the same app through a bounded `Application.Run()` validation that opens via `StartupUri`, validates the WPF object graph, startup event resources, and shuts down automatically:
