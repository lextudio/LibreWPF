...
- WPF app resources, menus, bindings, routed commands, input bindings, value converters, `MultiBinding`, `CollectionViewSource` sorting/grouping/filtering, resource `DataTemplate`s, `DataTemplateSelector`, `ItemContainerStyle`, `ItemsPanelTemplate`, compiled `ControlTemplate` styles, `ValidationRule`, `BindingGroup`, `Validation.ErrorTemplate`, reusable `UserControl`, `Frame`/`Page` navigation, list/table/tree controls, and a basic `FlowDocument`
