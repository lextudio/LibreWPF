...
- WPF app resources, menus, bindings, routed commands, input bindings, value converters, `MultiBinding`, `CollectionViewSource` sorting/grouping/filtering, resource `DataTemplate`s, `DataTemplateSelector`, `ItemContainerStyle`, `ItemsPanelTemplate`, compiled `ControlTemplate` styles, reusable `UserControl`, `Frame`/`Page` navigation, list/table/tree controls, and a basic `FlowDocument`
