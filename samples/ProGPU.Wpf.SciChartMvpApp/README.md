...
The 2D panel exercises native line, mountain, column, financial, shaped heatmap, and colored sprite marker paths. The colored marker path maps SciChart-style `DrawColoredSprites(...)` start/count semantics to ProGPU-owned textured quad draws with active clipping and per-marker ARGB tinting.
