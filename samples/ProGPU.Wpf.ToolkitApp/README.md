...
The XAML uses regular `http://schemas.xceed.com/wpf/xaml/toolkit` and `http://schemas.xceed.com/wpf/xaml/avalondock` namespaces. It validates `Xceed.Wpf.Toolkit` controls, `Xceed.Wpf.AvalonDock.DockingManager`, AvalonDock documents, icon-backed document headers, anchorables, left/right auto-hide side groups, theme assembly loading, compiled BAML, bindings, code-behind event hookup, document activation, anchorable hide/show state, public `ToggleAutoHide()` transitions, and XML layout serialization/deserialization without app-side ProGPU APIs.
