...
The XAML uses regular `http://schemas.xceed.com/wpf/xaml/toolkit` and `http://schemas.xceed.com/wpf/xaml/avalondock` namespaces. It validates `Xceed.Wpf.Toolkit` controls, popup-backed editors (`CheckComboBox`, `TimePicker`, `ColorPicker`, `CalculatorUpDown`, and `DropDownButton`), text/editing controls (`WatermarkTextBox`, `MaskedTextBox`, and `RichTextBox` with `PlainTextFormatter`), `CheckListBox` selection, `PropertyGrid`, `BusyIndicator`, `Xceed.Wpf.AvalonDock.DockingManager`, AvalonDock documents, `LayoutDocument.Float()`/`DockAsDocument()` transitions, icon-backed document headers, anchorables, left/right auto-hide side groups, theme assembly loading, compiled BAML, bindings, code-behind event hookup, document activation, floating-window model state, anchorable hide/show state, public `ToggleAutoHide()` transitions, and XML layout serialization/deserialization without app-side ProGPU APIs.
